...
// Load this before admin.html or user.html
// ============================================================
...
DLS.fixtures = [
  { id:1, homeId:1, awayId:2, time:"7:00 PM", date:"Today",    status:"FT",   homeGoals:2, awayGoals:1, highlighted:true,
    events:[
      {min:23,teamId:1,player:"Mugisha Eric",type:"goal"},
      {min:55,teamId:1,player:"Habimana Jean",type:"goal"},
      {min:67,teamId:2,player:"Nkurunziza David",type:"goal"}
    ],
    chat:[
      {user:"Eric M.",msg:"LET'S GOOO 🔥",time:"7:05 PM"},
      {user:"David N.",msg:"Game on! 💪",time:"7:06 PM"},
      {user:"Paul B.",msg:"Watching this one 👀",time:"7:10 PM"}
    ]
  },
  { id:2, homeId:3, awayId:4, time:"8:00 PM", date:"Today",    status:"FT",   homeGoals:1, awayGoals:3, highlighted:false,
    events:[
      {min:10,teamId:4,player:"Rwabuhungu Sam",type:"goal"},
      {min:30,teamId:3,player:"Nsengimana Felix",type:"goal"},
      {min:71,teamId:4,player:"Rwabuhungu Sam",type:"goal"},
      {min:89,teamId:4,player:"Ndayishimiye Tom",type:"goal"}
    ],
    chat:[]
  },
...
DLS.nextId = arr => arr.length ? Math.max(...arr.map(x=>x.id)) + 1 : 1;
...
// Notify all players of a team
DLS.notifyTeam = function(teamId, msg, type) {
  const u = DLS.users.find(u => u.teamId === teamId);
  if(u) DLS.notifications.push({ id:DLS.nextId(DLS.notifications), userId:u.id, msg, read:false, time:'Just now', type:type||'match' });
};

// Notify all users
DLS.notifyAll = function(msg, type) {
  DLS.users.forEach(u => {
    DLS.notifications.push({ id:DLS.nextId(DLS.notifications), userId:u.id, msg, read:false, time:'Just now', type:type||'match' });
  });
};
